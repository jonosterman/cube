...
public class Sha4J {
	...
	public byte[] sha1Digest(InputStream inputStream) throws IOException {
		...
		for (;;) {
			...
			int j = 0;
			for (; i < 16; j += 4, i++) {
				M[i] = (Mb[j] << 24) | (((Mb[j + 1]) & 0xff) << 16) | (((Mb[j + 2]) & 0xff) << 8) | ((Mb[j + 3]) & 0xff);
			}
			...
			int t = 0;
			for (; t < 80; t++) {
				int T = modulo32Add(modulo32Add(modulo32Add(ROTL(5, a), f(t, b, c, d)), modulo32Add(e, K(t))), W[t]);
				e = d;
				d = c;
				c = ROTL(30, b);
				b = a;
	...
	private byte[] sha256Digest(InputStream inputStream, boolean sha224) throws IOException {
		...
		for (;;) {
			...
			int t = 0;
			for (; t < 64; t++) {
				int T1 = modulo32Add(modulo32Add(modulo32Add(h, SIGMA256_1(e)), modulo32Add(Ch(e, f, g), K256[t])), W[t]);
	...
	private void doPadBlock256(int[] H, int[] M, int[] W, boolean addOne) {
		...
		int T2 = 0;
		for (; t < 64; t++) {
			T1 = modulo32Add(modulo32Add(modulo32Add(h, SIGMA256_1(e)), modulo32Add(Ch(e, f, g), K256[t])), W[t]);
	...
	private static final int[] K256 = { 0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01,
	...
	private void prepareMessageSchedule256(int[] M, int[] W) {
		for (int t = 0; t < 64; t++) {
			if (t < 16) {
				...
				W[t] = modulo32Add(modulo32Add(sigma256_1(W[t - 2]), W[t - 7]), modulo32Add(sigma256_0(W[t - 15]), W[t - 16]));
			}
		}
	}
	...
	private byte[] sha512Digest(InputStream inputStream, boolean sha384) throws IOException {
		// read bytes
		...
		for (;;) {
			...
			int j = 0;
			for (; i < 16; j += 8, i++) {
				M[i] = (((long) Mb[j]) << 56) | ((long) ((Mb[j + 1]) & 0xff) << 48) | ((long) ((Mb[j + 2]) & 0xff) << 40) | ((long) ((Mb[j + 3]) & 0xff) << 32)
						| ((long) ((Mb[j + 4]) & 0xff) << 24) | ((long) ((Mb[j + 5]) & 0xff) << 16) | ((long) ((Mb[j + 6]) & 0xff) << 8)
						| ((long) (Mb[j + 7]) & 0xff);
			}
			// prepare message schedule
			...
			int t = 0;
			for (; t < 80; t++) {
				long T1 = modulo64Add(modulo64Add(h, SIGMA512_1(e)), modulo64Add(modulo64Add(Ch(e, f, g), K512[t]), W[t]));
	...
	private void doPadBlock512(long[] H, long[] M, long[] W, boolean addOne) {
		...
		long T2 = 0;
		for (; t < 80; t++) {
			T1 = modulo64Add(modulo64Add(modulo64Add(modulo64Add(h, SIGMA512_1(e)), Ch(e, f, g)), K512[t]), W[t]);
	...
	private static final long[] K512 = { 0x428a2f98d728ae22L, 0x7137449123ef65cdL, 0xb5c0fbcfec4d3b2fL, 0xe9b5dba58189dbbcL, 0x3956c25bf348b538L,
			0x59f111f1b605d019L, 0x923f82a4af194f9bL, 0xab1c5ed5da6d8118L, 0xd807aa98a3030242L, 0x12835b0145706fbeL, 0x243185be4ee4b28cL, 0x550c7dc3d5ffb4e2L,
			0x72be5d74f27b896fL, 0x80deb1fe3b1696b1L, 0x9bdc06a725c71235L, 0xc19bf174cf692694L, 0xe49b69c19ef14ad2L, 0xefbe4786384f25e3L, 0x0fc19dc68b8cd5b5L,
			0x240ca1cc77ac9c65L, 0x2de92c6f592b0275L, 0x4a7484aa6ea6e483L, 0x5cb0a9dcbd41fbd4L, 0x76f988da831153b5L, 0x983e5152ee66dfabL, 0xa831c66d2db43210L,
			0xb00327c898fb213fL, 0xbf597fc7beef0ee4L, 0xc6e00bf33da88fc2L, 0xd5a79147930aa725L, 0x06ca6351e003826fL, 0x142929670a0e6e70L, 0x27b70a8546d22ffcL,
			0x2e1b21385c26c926L, 0x4d2c6dfc5ac42aedL, 0x53380d139d95b3dfL, 0x650a73548baf63deL, 0x766a0abb3c77b2a8L, 0x81c2c92e47edaee6L, 0x92722c851482353bL,
			0xa2bfe8a14cf10364L, 0xa81a664bbc423001L, 0xc24b8b70d0f89791L, 0xc76c51a30654be30L, 0xd192e819d6ef5218L, 0xd69906245565a910L, 0xf40e35855771202aL,
			...
			0x5b9cca4f7763e373L, 0x682e6ff3d6b2b8a3L, 0x748f82ee5defb2fcL, 0x78a5636f43172f60L, 0x84c87814a1f0ab72L, 0x8cc702081a6439ecL, 0x90befffa23631e28L,
			0xa4506cebde82bde9L, 0xbef9a3f7b2c67915L, 0xc67178f2e372532bL, 0xca273eceea26619cL, 0xd186b8c721c0c207L, 0xeada7dd6cde0eb1eL, 0xf57d4f7fee6ed178L,
			0x06f067aa72176fbaL, 0x0a637dc5a2c898a6L, 0x113f9804bef90daeL, 0x1b710b35131c471bL, 0x28db77f523047d84L, 0x32caab7b40c72493L, 0x3c9ebe0a15c9bebcL,
	...
	private void prepareMessageSchedule512(long[] M, long[] W) {
		for (int t = 0; t < 80; t++) {
			if (t < 16) {
				...
				W[t] = modulo64Add(modulo64Add(sigma512_1(W[t - 2]), W[t - 7]), modulo64Add(sigma512_0(W[t - 15]), W[t - 16]));
			}
		}
	}
	...
	/**
	 * Resets instance for use in other operation. This must be called before
	 * each consequent operation. If not called results are not correct.
	 */
	public void reset() {
		// sha-1 variables

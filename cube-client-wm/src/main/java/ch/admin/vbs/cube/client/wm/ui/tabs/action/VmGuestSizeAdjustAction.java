...
/**
 * This class handles the standby action for a virtual machine. When the action
 * is invoked, all listeners will be informed.
 * 
 * 
 * 
 */
public class VmGuestSizeAdjustAction extends VmAbstractAction {
	private static final long serialVersionUID = 3185872544856252979L;
	private final IUserInterface userUI;

	/**
	 * Constructs the vm standby action which will invoke all listeners, when
	 ...
	 */
	public VmGuestSizeAdjustAction(VmHandle h,  IUserInterface userUI) {
		super(I18nBundleProvider.getBundle().getString("vm.action.guestSizeAdjust.text"), IconManager.getInstance().getIcon("screens_icon16.png"), h);
		this.userUI = userUI;
			setEnabled(enabled);
	}
	...
	public void exec(ActionEvent actionEvent) {
		userUI.adjustGuestSize(getVmHandle().getVmId());
	}
}

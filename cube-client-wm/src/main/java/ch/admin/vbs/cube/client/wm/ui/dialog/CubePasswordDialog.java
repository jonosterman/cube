 ...
 * dialog.
 */
public class CubePasswordDialog extends CubeWizard {
	/** Logger */
	private static final Logger LOG = LoggerFactory.getLogger(CubePasswordDialog.class);
	...
	private LinkedList<CubePasswordDialogListener> listeners;
	...
	protected JPanel createCenterContentPanel() {
		...
		JLabel labelLbl = new JLabel(bundle.getString("login.pin"));
		// layout
	 ...
	 *            the listener
	 */
	public void addPasswordDialogListener(CubePasswordDialogListener listener) {
	 ...
	 *            the listener
	 */
	public void removePasswordDialogListener(CubePasswordDialogListener listener) {
		synchronized (listener) {
	...
	private void notifyListenersAndClose(final boolean canceled) {
		...
		execs.execute(new Runnable() {
			...
			@Override
			public void run() {
				List<CubePasswordDialogListener> cloned;
				synchronized (listeners) {
					cloned = (List<CubePasswordDialogListener>) listeners.clone();
				}
				for (CubePasswordDialogListener listener : cloned) {
					if (canceled) {
						listener.quit(null);
					} else {
	...
	public ButtonPanel createButtonPanel() {
		okButton = addWizardAction(new AbstractAction(bundle.getString("passworddialog.button.login")) {
